...
const editCourse = async (req, res) => {
    ...
    CourseModel.updateCourseById(req.params.id, req.body.converted)
        .then(result => {
            if (result.nModified == 1 && result.ok == 1 && result.n == 1)
                return res.status(204).send({ success: true, message: 'course updated successfully.' })
            else
                return res.status(400).send({ success: false, message: 'course could not be updated.' })
...
module.exports.getCoursesBy = getCoursesBy;
